...
if [[ -z "${MCW_GITHUB_URL}" ]]; then
    MCW_GITHUB_URL=https://$MCW_GITHUB_TOKEN@github.com/JosueVidalMVP/Fabmedical.git
fi
...
if [[ -z "${MCW_SECONDARY_LOCATION}" ]]; then
    MCW_SECONDARY_LOCATION="eastus2"
    MCW_SECONDARY_LOCATION_NAME="East US2"
fi
...
if [[ ! -e ~/Fabmedical ]]; then
    ...
    cd ~/Fabmedical
    git init
    git remote add origin git@github.com:JosueVidalMVP/Fabmedical.git
fi
...
VM_PUBLIC_IP=$(az vm show -d -g fabmedical-solvidal -n fabmedical-$MCW_SUFFIX --query publicIps -o tsv)
